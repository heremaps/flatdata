...
DOCOPT_INLINE
std::ostream&
docopt::operator<<( std::ostream& os, value const& val )
{
    if ( val.isBool( ) )
    {
        bool b = val.asBool( );
...
public:
    ...
    static Tokens
    from_pattern( std::string const& source )
    {
        ...
        std::for_each( std::sregex_iterator{ source.begin( ), source.end( ), re_separators },
                       std::sregex_iterator{ }, [ & ]( std::smatch const& match ) {
                           // handle anything before the separator (this is the "stuff" between the
                           // delimeters)
...
template < typename T >
std::vector< T* >
flat_filter( Pattern& pattern )
{
    std::vector< Pattern* > flattened = pattern.flat(
        []( Pattern const* p ) -> bool { return dynamic_cast< T const* >( p ) != nullptr; } );
...
parse_section( std::string const& name, std::string const& source )
{
    // ECMAScript regex only has "?=" for a non-matching lookahead. In order to make sure we always
    // have a newline to anchor our matching, we have to avoid matching the final newline of each
    // grouping. Therefore, our regex is adjusted from the docopt Python one to use ?= to match the
    // newlines before the following lines, rather than after.
...
static bool
is_argument_spec( std::string const& token )
{
    if ( token.empty( ) )
        return false;
...
static PatternList
parse_long( Tokens& tokens, std::vector< Option >& options )
{
    // long ::= '--' chars [ ( ' ' | '=' ) chars ] ;
...
static PatternList
parse_short( Tokens& tokens, std::vector< Option >& options )
{
    // shorts ::= '-' ( chars )* [ [ ' ' ] chars ] ;
...
static PatternList
parse_atom( Tokens& tokens, std::vector< Option >& options )
{
...
static PatternList
parse_seq( Tokens& tokens, std::vector< Option >& options )
{
    // seq ::= ( atom [ '...' ] )* ;"""
...
maybe_collapse_to_required( PatternList&& seq )
{
    if ( seq.size( ) == 1 )
    {
...
static std::shared_ptr< Pattern >
maybe_collapse_to_either( PatternList&& seq )
{
    if ( seq.size( ) == 1 )
    {
        return std::move( seq[ 0 ] );
...
PatternList
parse_expr( Tokens& tokens, std::vector< Option >& options )
{
    // expr ::= seq ( '|' seq )* ;
...
static Required
parse_pattern( std::string const& source, std::vector< Option >& options )
{
...
static std::string
formal_usage( std::string const& section )
{
    std::string ret = "(";
...
static PatternList
parse_argv( Tokens tokens, std::vector< Option >& options, bool options_first )
{
...
std::vector< Option >
parse_defaults( std::string const& doc )
{
    ...
    static std::regex const re_delimiter{
        ...
        "(?=-{1,2})"  // [split happens here] (positive lookahead) ... and followed by one or two
                      // hyphes
    };
...
static bool
isOptionSet( PatternList const& options, std::string const& opt1, std::string const& opt2 = "" )
{
...
static void
extras( bool help, bool version, PatternList const& options )
{
