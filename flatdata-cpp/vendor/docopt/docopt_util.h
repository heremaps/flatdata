...
std::string
trim( std::string&& str, const std::string& whitespace = " \t\n" )
{
...
split( std::string const& str, size_t pos = 0 )
{
...
template < typename I >
std::string
join( I iter, I end, std::string const& delim )
{
    if ( iter == end )
        return { };
...
std::vector< std::string >
regex_split( std::string const& text, std::regex const& re )
{
    std::vector< std::string > ret;
