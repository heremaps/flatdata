...
struct PatternHasher
{
    template < typename P >
    size_t
    operator( )( std::shared_ptr< P > const& pattern ) const
    {
        return pattern->hash( );
    }
    template < typename P >
    size_t
    operator( )( P const* pattern ) const
    {
        return pattern->hash( );
    }
    template < typename P >
...
struct PatternPointerEquality
{
    ...
    bool
    operator( )( std::shared_ptr< P1 > const& p1, std::shared_ptr< P2 > const& p2 ) const
    {
...
using UniquePatternSet
    = std::unordered_set< std::shared_ptr< Pattern >, PatternHasher, PatternPointerEquality >;
...
public:
    ...
    // Attempt to find something in 'left' that matches this pattern's spec, and if so, move it to
    // 'collected'
    virtual bool match( PatternList& left,
...
public:
    ...
    virtual void
    collect_leaves( std::vector< LeafPattern* >& lst ) override final
    {
        lst.push_back( this );
    }
...
protected:
    virtual std::pair< size_t, std::shared_ptr< LeafPattern > > single_match(
        PatternList const& ) const = 0;
...
public:
    ...
    void
    setChildren( PatternList children )
    {
        fChildren = std::move( children );
    }
    ...
    virtual void
    fix_identities( UniquePatternSet& patterns )
    {
        for ( auto& child : fChildren )
        {
...
protected:
    virtual std::pair< size_t, std::shared_ptr< LeafPattern > > single_match(
        PatternList const& left ) const override;
};
...
static inline std::vector< PatternList >
transform( PatternList pattern )
{
    ...
    while ( !groups.empty( ) )
    {
        ...
        auto child_iter = std::find_if( children.begin( ), children.end( ),
...
inline bool
LeafPattern::match( PatternList& left,
                    std::vector< std::shared_ptr< LeafPattern > >& collected ) const
{
    ...
    auto same_name = std::find_if(
        ...
        [ & ]( std::shared_ptr< LeafPattern > const& p ) { return p->name( ) == name( ); } );
    if ( getValue( ).isLong( ) )
    {
        long val = 1;
...
inline std::pair< size_t, std::shared_ptr< LeafPattern > >
...
inline Option
Option::parse( std::string const& option_description )
{
...
{
    auto thematch
        = find_if( left.begin( ), left.end( ), [ this ]( std::shared_ptr< Pattern > const& a ) {
    ...
    return { std::distance( left.begin( ), thematch ),
             std::dynamic_pointer_cast< LeafPattern >( *thematch ) };
}

inline bool
Required::match( PatternList& left, std::vector< std::shared_ptr< LeafPattern > >& collected ) const
{
    auto l = left;
    auto c = collected;
    for ( auto const& pattern : fChildren )
...
inline bool
OneOrMore::match( PatternList& left,
                  std::vector< std::shared_ptr< LeafPattern > >& collected ) const
{
...
inline bool
Either::match( PatternList& left, std::vector< std::shared_ptr< LeafPattern > >& collected ) const
{
