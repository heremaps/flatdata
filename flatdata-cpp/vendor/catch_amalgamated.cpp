                 ...
                 std::vector< double >::iterator last )
{
    ...
    auto n = static_cast< int >( last - first );  // seriously, one can't use integral types without
                                                  // hell in C++
...
public:  // IMutableRegistryHub
    void
    registerReporter( std::string const& name, IReporterFactoryPtr factory ) override
    {
    ...
    void
    registerTest( Detail::unique_ptr< TestCaseInfo >&& testInfo,
...
Detail::InternalParseResult
Parser::parse( std::string const& exeName, Detail::TokenStream const& tokens ) const
{
    struct ParserInfo
    {
    ...
    auto result
        = Detail::InternalParseResult::ok( Detail::ParseState( ParseResultType::NoMatch, tokens ) );
...
private:
    void
    use( Colour::Code ) const override
    {
    }
};
}  // namespace
...
public:
    ...
    static bool
    useImplementationForStream( IStream const& stream )
    {
...
private:
    void
    use( Colour::Code _colourCode ) const override
    {
        switch ( _colourCode )
        {
        case Colour::None:
    ...
    void
    setTextAttribute( WORD _textAttribute ) const
    {
...
public:  // IMutableContext
    ...
    void
    setConfig( IConfig const* config ) override
    {
        m_config = config;
    }
...
void
ReporterRegistry::registerReporter( std::string const& name, IReporterFactoryPtr factory )
{
...
void
ReporterRegistry::registerListener( Detail::unique_ptr< EventListenerFactory > factory )
{
...
splitKVPair( StringRef kvString )
{
    auto splitPos = static_cast< size_t >(
        std::distance( kvString.begin( ), std::find( kvString.begin( ), kvString.end( ), '=' ) ) );
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker
{
    ...
    acquire( TrackerContext& ctx, TestCaseTracking::NameAndLocation const& nameAndLocation )
    {
        ...
        {
            auto newTracker = Catch::Detail::make_unique< GeneratorTracker >( nameAndLocation, ctx,
                                                                              &currentTracker );
...
RunContext::runTest( TestCaseHandle const& testCase )
{
    ...
    do
    {
        ...
        auto statsForOneRun
            = TestCaseStats( testInfo, singleRunTotals, oneRunCout, oneRunCerr, aborting( ) );
...
auto
RunContext::acquireGeneratorTracker( StringRef generatorName, SourceLineInfo const& lineInfo )
    -> IGeneratorTracker&
{
...
{
    auto testInfoCmp
        = []( TestCaseInfo const* lhs, TestCaseInfo const* rhs ) { return *lhs < *rhs; };
...
SectionTracker::acquire( TrackerContext& ctx, NameAndLocation const& nameAndLocation )
{
    ...
    {
        auto newSection
            = Catch::Detail::make_unique< SectionTracker >( nameAndLocation, ctx, &currentTracker );
...
std::string
NoneTrueMatcher::describe( ) const
{
...
void
ReporterBase::listReporters( std::vector< ReporterDescription > const& descriptions )
{
    defaultListReporters( m_stream, descriptions, m_config->verbosity( ) );
}

void
ReporterBase::listListeners( std::vector< ListenerDescription > const& descriptions )
{
...
printTotals( std::ostream& out, const Totals& totals, ColourImpl* colourImpl )
{
    ...
    else if ( totals.assertions.total( ) == 0 )
    {
        out << "Passed " << bothOrAll( totals.testCases.total( ) )
            << pluralise( totals.testCases.total( ), "test case"_sr ) << " (no assertions).";
    }
...
private:
    ...
    void
    printResultType( Colour::Code colour, StringRef passOrFail ) const
    {
        if ( !passOrFail.empty( ) )
        {
    ...
    void
    printIssue( char const* issue ) const
    {
        stream << ' ' << issue;
    }
    ...
    void
    printRemainingMessages( Colour::Code colour = compactDimColour )
    {
        if ( itMessage == messages.end( ) )
            return;
...
ConsoleReporter::printTotals( Totals const& totals )
{
    ...
    {
        m_stream << m_colour->guardColour( Colour::ResultSuccess ) << "All tests passed";
        m_stream << " (" << pluralise( totals.assertions.passed, "assertion"_sr ) << " in "
    ...
    for ( auto col : cols )
    {
        ...
        else if ( value != "0" )
        {
            m_stream << m_colour->guardColour( Colour::LightGrey ) << " | "
                     << m_colour->guardColour( col.colour ) << value << ' ' << col.label;
        }
    }
    m_stream << '\n';
}
...
void
defaultListListeners( std::ostream& out, std::vector< ListenerDescription > const& descriptions )
{
...
void
defaultListTags( std::ostream& out, std::vector< TagInfo > const& tags, bool isFiltered )
{
    if ( isFiltered )
    {
...
static void
normalizeNamespaceMarkers( std::string& str )
{
...
void
MultiReporter::testCasePartialStarting( TestCaseInfo const& testInfo, uint64_t partNumber )
{
    for ( auto& reporterish : m_reporterLikes )
    {
...
void
MultiReporter::testCasePartialEnded( TestCaseStats const& testStats, uint64_t partNumber )
{
...
void
MultiReporter::listReporters( std::vector< ReporterDescription > const& descriptions )
{
    for ( auto& reporterish : m_reporterLikes )
    {
...
void
MultiReporter::listListeners( std::vector< ListenerDescription > const& descriptions )
{
    for ( auto& reporterish : m_reporterLikes )
    {
...
void
SonarQubeReporter::writeTestFile( std::string const& filename,
...
void
SonarQubeReporter::writeAssertions( SectionNode const& sectionNode, bool okToFail )
{
    for ( auto const& assertionOrBenchmark : sectionNode.assertionsAndBenchmarks )
    {
        if ( assertionOrBenchmark.isAssertion( ) )
...
private:
    void
    printResultType( StringRef passOrFail ) const
    {
        if ( !passOrFail.empty( ) )
        {
    ...
    void
    printIssue( StringRef issue ) const
    {
        stream << ' ' << issue;
    }
    ...
    void
    printRemainingMessages( Colour::Code colour = tapDimColour )
    {
        if ( itMessage == messages.end( ) )
        {
            return;
    ...
    if ( !result.isOk( ) )
    {
        ...
        if ( currentTestCaseInfo->okToFail( ) )
        {
            ...
            m_stream << "##teamcity[testIgnored"
                     << " name='" << escape( currentTestCaseInfo->name ) << '\'' << " message='"
                     << escape( msg.str( ) ) << '\'' << "]\n";
        }
        else
        {
...
XmlReporter::sectionStarting( SectionInfo const& sectionInfo )
{
    ...
    if ( m_sectionDepth++ > 0 )
    {
        m_xml.startElement( "Section" ).writeAttribute( "name"_sr, trim( sectionInfo.name ) );
...
XmlReporter::testCaseEnded( TestCaseStats const& testCaseStats )
{
    ...
    if ( !testCaseStats.stdOut.empty( ) )
        m_xml.scopedElement( "StdOut" )
            .writeText( trim( testCaseStats.stdOut ), XmlFormatting::Newline );
    if ( !testCaseStats.stdErr.empty( ) )
        m_xml.scopedElement( "StdErr" )
            .writeText( trim( testCaseStats.stdErr ), XmlFormatting::Newline );
