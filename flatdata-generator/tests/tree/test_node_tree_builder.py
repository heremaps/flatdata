...
from flatdata.generator.tree.builder import _build_node_tree, _compute_structure_sizes, resolve_references, _update_field_type_references
# pylint: disable=unused-wildcard-import
from flatdata.generator.tree.errors import *


def check_constant(c, typename, value):
    assert isinstance(c, nodes.Constant)
    assert typename == c.type.name
    assert isinstance(c.type, BasicType)
    assert value == c.value


def check_field(f, typename, width, offset):
    assert isinstance(f, nodes.Field)
    assert isinstance(f.type, BasicType)
    assert typename == f.type.name
    assert offset == f.offset
    assert width == f.type.width


def check_struct(s, size_in_bits, size_in_bytes):
    assert isinstance(s, nodes.Structure)
    assert size_in_bits == s.size_in_bits
    assert size_in_bytes == s.size_in_bytes


def check_explicit_reference(r, source_type, source_field, destination):
    assert "explicit_reference" in r
    ref = r.explicit_reference
    assert source_type == ref.source_type
    assert source_field == ref.source_field
    assert destination == ref.destination


def check_bound_resource(b, name, refnames):
    assert isinstance(b, resources.BoundResource)
    assert len(refnames) == len(b.children)
    assert name == b.name
    for i, ref in enumerate(refnames):
        refnode = b.children[i]
        assert isinstance(refnode, refs.ResourceReference)
        assert ref == refnode.target


def test_empty_schema_returns_none():
    tree = _build_node_tree("")
    assert set() == tree.symbols()
...
def test_empty_schema_with_namespace_parsed_correctly():
    ...
    assert isinstance(root.find(".foobar"), nodes.Namespace)
...
def test_empty_schema_with_nested_namespace_parsed_correctly():
    ...
    for path, name in [(".foo", "foo"), (".foo.bar", "bar"), (".foo.bar.baz", "baz")]:
        node = tree.find(path)
        assert isinstance(node, nodes.Namespace)
...
def test_constants_are_placed_on_final_namespace():
    ...
    assert {".a", ".a.b", ".a.b.foo"} == tree.symbols()
    assert isinstance(tree.find(".a.b.foo"), nodes.Constant)
...
def test_constants_are_parsed():
    tree = _build_node_tree("""namespace foo{
        ...
        """)
    assert {".foo", ".foo.bar", ".foo.foo", ".foo.foo2", ".foo.bar2"} == tree.symbols()
...
def constant_size_check():
    with pytest.raises(InvalidConstantValueError):
        _build_node_tree("""namespace a{
            const i8 foo = 128;
            }
            """)
...
def test_single_structure_is_parsed_correctly():
    ...
    assert {".foo", ".foo.Bar", ".foo.Bar.fieldB", ".foo.Bar.fieldA"} == tree.symbols()
...
def test_two_structures_are_parsed_correctly():
    tree = _build_node_tree("""namespace foo{
        ...
        """)
    _compute_structure_sizes(tree)
    assert {".foo", ".foo.Baz", ".foo.Baz.fieldB", ".foo.Bar", ".foo.Bar.fieldA"} == tree.symbols()
...
def test_archives_resources_references_are_populated(test_case):
    def __test(schema, references):
        tree = _build_node_tree("""namespace foo{
            ...
            """ % schema)
        assert {".foo", ".foo.A", ".foo.A.resourceA"}.union(references) == tree.symbols()
...
def test_archives_archive_resources_references_are_populated():
    tree = _build_node_tree("""namespace foo{
        ...
        """)
    assert {".foo", ".foo.A", ".foo.A.resourceA", ".foo.A.resourceA.@baz@B"} == tree.symbols()
    assert isinstance(tree.find(".foo.A.resourceA.@baz@B"), refs.TypeReference)
...
def test_multi_vector_builtin_types_are_correct():
    ...
    assert 33 == res.width
    assert 1 == len(res.builtins)

    index_type = res.builtins[0]
    _compute_structure_sizes(index_type)
    assert {"IndexType33", "IndexType33.value"} == index_type.symbols()
...
def test_explicit_reference_decoration_spawns_all_the_right_references():
    ...
    assert 2 == len(r.decorations)
    check_explicit_reference(r.decorations[0], "A", "refB", "resourceB")
    check_explicit_reference(r.decorations[1], "A", "refC", "resourceC")

    assert isinstance(tree.find(".foo.X.resourceA.er_A_refB_resourceB.@A@refB"),
                       refs.FieldReference)
    assert isinstance(tree.find(".foo.X.resourceA.er_A_refB_resourceB.@resourceB"),
                       refs.ResourceReference)
    assert isinstance(tree.find(".foo.X.resourceA.er_A_refC_resourceC.@A@refC"),
                       refs.FieldReference)
    assert isinstance(tree.find(".foo.X.resourceA.er_A_refC_resourceC.@resourceC"),
                       refs.ResourceReference)
...
def test_optional_decoration():
    ...
    assert 1 == len(r.decorations)
    assert "optional" in r.decorations[0]
    assert r.optional
...
def test_multiple_archives_can_be_defined():
    ...
    assert {".foo", ".foo.S", ".foo.S.x", ".foo.A", ".foo.A.resource1", ".foo.A.resource1.@S",
                  ".foo.F", ".foo.F.x", ".foo.B", ".foo.B.resource0", ".foo.B.resource0.@F",
                  ".foo.B.resource1",
                  ".foo.B.resource1.@S"} == tree.symbols()
...
def test_namespace_merging_works_for_separate_namespaces():
    ...
    assert {'.n1', '.n1.A', '.n1.A.refN1',
                  '.n2', '.n2.A', '.n2.A.refN2'} == tree.symbols()
...
def test_namespace_merging_works_for_nested_namespaces():
    ...
    assert {'.n1', '.n1.A', '.n1.A.refN1',
                  '.n1.n2', '.n1.n2.A', '.n1.n2.A.refN2'} == tree.symbols()
...
def test_namespace_merging_works_for_same_namespaces_with_different_symbols():
    ...
    assert {'.n1', '.n1.n2',
                  ...
                  '.n1.n2.B', '.n1.n2.B.refN2'} == tree.symbols()
...
def test_duplicate_enum_value():
    with pytest.raises(DuplicateEnumValueError):
        _build_node_tree("""
        namespace n {
            enum A : u16 : 3 {
                VALUE_1 = 1,
...
def test_not_enough_bits_in_enum_field():
    with pytest.raises(InvalidEnumWidthError):
        tree = _build_node_tree("""
        namespace n {
            enum A : i16 : 8 {
                VALUE_1 = 127
